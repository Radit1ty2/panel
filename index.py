...
def create_moving_text_window():
    ...
    def on_closing():
        ...
        window_open = False
        root.destroy()
        # Tidak langsung reopen, beri jeda
        root.after(1000, reopen_window)
    ...
    root = tk.Tk()
    root.title("By Gusti raditya bhaskara")
    root.geometry("800x100")
    root.resizable(False, False)

    canvas = tk.Canvas(root, width=800, height=100, bg="black")
    canvas.pack()

    text = canvas.create_text(400, 50, 
                            text="Rakanashi DDOS tools", 
    ...
    def move_text():
        ...
        x_pos = canvas.coords(text)[0]
        if x_pos > 1000:  # Reset ketika keluar layar
            canvas.coords(text, -600, 50)
...
def main():
    ...
    # Create moving text window in separate thread
    window_thread = threading.Thread(target=create_moving_text_window, daemon=True)
    window_thread.start()
    
    # Beri waktu untuk window muncul
    time.sleep(1)
